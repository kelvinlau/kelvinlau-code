...
void Make_Graph() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    do {
      ...
      if (u < v) {
        E[M].u = u - p, E[M].v = v - p;
        E[M++].w = sqrt((double)(u->x - v->x) * (u->x - v->x) +
...
void Low_tangent(edge *e_l, point *o_l, edge *e_r, point *o_r,
    edge **l_low, point **OL, edge **r_low, point **OR) {
  point *d_l = Other(e_l, o_l), *d_r = Other(e_r, o_r);
  while (1) {
    if (C3(o_l, d_l, o_r) > 0.0)
      e_l = Prev(e_l, d_l), o_l = d_l, d_l = Other(e_l, o_l);
    else if (C3(o_r, d_r, o_l) < 0.0)
  ...
  *tangent = B = Join(L, OL, R, OR, 0);
  ...
  while (1) {
    ...
    double l1, l2, l3, l4, r1, r2, r3, r4,
           cot_N, cot_P, cot_L, cot_R, u1, v1, u2, v2;
    V(l, O, l1, l2);
    V(l, D, l3, l4);
    V(r, O, r1, r2);
    V(r, D, r3, r4);
...
void Divide(int s, int t, edge **L, edge **R) {
  int n = t - s + 1;
  if (n == 2)
    *L = *R = Make_edge(Q[s], Q[t]);
  else if (n == 3) {
    edge *a = Make_edge(Q[s], Q[s + 1]), *b = Make_edge(Q[s + 1], Q[t]), *c;
    Splice(a, b, Q[s + 1]);
    double v = C3(Q[s], Q[s + 1], Q[t]);
    if (v > 0.0)
      c = Join(a, Q[s], b, Q[t], 0), *L = a, *R = b;
    else if (v < 0.0)
      c = Join(a, Q[s], b, Q[t], 1), *L = c, *R = c;
    else
      *L = a, *R = b;
  } else if (n > 3) {
    int split = (s + t) / 2;

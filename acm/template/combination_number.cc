...
int getPrime() {
  memset(prime, 0, sizeof(prime[0]) * (PRIMERANGE + 1));
...
T combinationModFac(T n, T k, T m) {
  ...
  for (int i = 0; i < facCnt; i++) {
    if (!i) {
      a = combinationModPtFac(n, k, factor[i][0], factor[i][2]);
      p = factor[i][1];
    } else {
      b = combinationModPtFac(n, k, factor[i][0], factor[i][2]);
      q = factor[i][1];
    }
    if (!i)
      continue;
...
T combinationModOdds(T n, T k, T m) {
  ...
  for (int i = 0; i < facCnt; i++) {
    if (!i) {
      a = combinationModPtAuto(n, k, factor[i][0], factor[i][2]);
      p = factor[i][1];
    } else {
      b = combinationModPtAuto(n, k, factor[i][0], factor[i][2]);
      q = factor[i][1];
    }
    if (!i)
      continue;
